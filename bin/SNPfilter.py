...
def SNP_filter(vcf_file,Sample_name,output_name,depth_ALT_cutoff, bowtie = True):
    vcf_file_list_freq = []
        ...
        if not lines.startswith("#") and not lines.startswith("CHR"):
            ...
            vcf_file_list_freq, vcf_file_list_freq_snp, oldPOS, oldPOS_out = \
                SNP_check_fq(lines_set,
                             ...
                             depth_ALT_cutoff, depth_cutoff, median_depth, oldPOS, oldPOS_out, bowtie)
    outputvcf(output_name,vcf_file_list_freq,vcf_file_list_freq_snp,Sample_name)

def load_sample_mapper(vcf_file):
    Sample_name = []
    mapper_sh = glob.glob(os.path.join(input_script + '/SNPcalling/',os.path.split(vcf_file)[-1].split('mapper')[0]
                                                                           +'*mapper*.sh'))
    for lines in open(mapper_sh[0], 'r'):
...
depth_ratio_cutoff1 = 0.08# for SNP at least depth_ratio_cutoff1 * median depth of neighbour_cov_range bp
depth_ALT_cutoff = 7 # for each REF + ALT freq
ALT_freq_cutoff =0.1 # for SNP depth of one ALT / total depth

output_name = 'final'
allvcf_file = glob.glob(os.path.join(args.i, '*%s' % (vcf_name)))
print(allvcf_file)
